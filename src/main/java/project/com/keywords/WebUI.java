...
import project.com.helpers.SystemHelpers;
import project.com.reports.AllureManager;
import project.com.reports.ExtentTestManager;
...
//        }
    ...
    public static void set2FAText(By by, String value) {
        waitForPageLoaded();
        waitForElementVisible(by);
        sleep(STEP_TIME);

        List<WebElement> codeFields = DriverManager.getDriver().findElements(by);
        for (int i = 0; i < value.length() && i < codeFields.size(); i++) {
            WebElement webElement =  codeFields.get(i);
